...
async function checkGameLimits() {
    try {
        // Check Crash Game limits
        const crashResponse = await fetch('/minigames/api/check-limit/crash_game');
        const crashData = await crashResponse.json();

        if (crashData.success && crashData.limit_check) {
            const limitInfo = crashData.limit_check;
...
window.confirmBetAndStartGame = async function() {
    ...
    // Start game UI immediately for instant response
        ...
        <div style="text-align: center; padding: 2rem;">
            ...
            <div style="margin-top: 2rem; display: flex; gap: 1rem; justify-content: center; align-items: center;">
                ...
                <div id="potentialWin" style="background: rgba(251, 191, 36, 0.2); padding: 1rem 2rem; border-radius: 12px; border: 2px solid #fbbf24;">
                    ...
                    <div style="font-size: 1.5rem; font-weight: 800; color: #fbbf24;">${betAmount.toFixed(2)} G$</div>
                </div>
            </div>
